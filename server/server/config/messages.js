module.exports = {
    ...
    authentication: {
        incorrect: {
            ...
            email: function (email) {
                return 'Email address \'' + email + '\' is not associated with an account';
            },
            password: 'Password is incorrect'
        },
        requires: {
            authentication: function (path) {
                return 'You must be authenticated to access ' + path;
            },
            role: function (path) {
                return 'You are not authorized to access ' + path;
            },
            self: function (username, path) {
                return 'You must be user \'' + username + '\' to access ' + path;
            }
        },
    ...
    post: {
        created: function (title) {
            return 'Post \'' + title + '\' was created';
        },
        deleted: function (title) {
            return 'Post \'' + title + '\' was deleted';
        },
        updated: function (title) {
            return 'Post \'' + title + '\' was updated';
        }
    },
    ...
    image: {
        ...
        exceedsFileSize: function (size) {
            return 'Image must be smaller than ' + size / 1024 / 1024 + ' MB';
        },
        mimeError: function (mime) {
            return 'File type \'' + mime + '\' is not supported';
        },
