(function ($) {
    ...
    var bookmarks = (function () {
        ...
        bookmarks.prototype = {content: '', openQueue: [], template: function (tree, recentTree) {
            var self = this;
            recentTree = {title: getI18nMsg('bookmarksRecent'), children: recentTree, id: 'recent'};
            ...
            self.content.find(".bookmarksManage").bind("click", function () {
                url = _browser.protocol + "://bookmarks/#1";
                if (_browser.liebao) {
            ...
            self.content.find('.bookMarksFolderItemAdd').unbind('click').bind('click', function () {
                ...
                if (DBOX.getLastDialbox() == "cloud") {
                    ...
                    PDI.appendDialbox('normal', toIndex, {title: title, url: url, isApp: false, isNew: true})
                } else {
                    PDI.insertDialbox('normal', {title: title, url: url, isApp: false, isNew: true})
                }
                DBOX.getBoxes();
                setTimeout(function () {
