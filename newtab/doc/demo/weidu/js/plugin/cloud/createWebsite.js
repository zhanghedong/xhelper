(function ($) {
    ...
    var createWebsite = (function (opt) {
        ...
        createWebsite.prototype = {suggestHide: true, trends: [], defaultLogoUrl: urlImg + 'ie_logo.png', defaultUrl: '', defaultTitle: '', url: '', title: '', LogoUrl: '', isUpdate: false, container: '', init: function () {
            var self = this;
            if (typeof chrome.history != "undefined" && isApp === false) {
                chrome.history.search({text: '', maxResults: 5000, startTime: 0}, function (data) {
                    if (data instanceof Array && data.length > 0) {
                        $.each(data, function (i, n) {
                            if (typeof n.url != "undefined" && n.url.indexOf('chrome-extension') == -1) {
                                try {
            ...
            self.container.find("#logoData").bind('change', function () {
                ...
                reader.onload = function (e) {
                    $.post(urlImg + "weidu/uploadLogo.php", {"imgData": this.result}, function (result) {
            ...
            self.container.find('#websiteForm').bind('submit', function () {
                ...
                if (self.isUpdate != false && self.container.find('#webSiteClassification').val() == cId) {
                    ...
                    if (updateOptions.length > 1) {
                        var sum = 0, index = 0;
                        PDI.updateDialbox(updateOptions[0], updateOptions[2], {title: self.container.find('#webSiteTitle').val(), url: self.container.find('#webSiteUrl').val(), img: self.container.find('#webSiteLogo').val(), isNew: true});
                        oauth.updateMsgId();
                        oauth.synchronize()
                    }
                    _isRefresh = "curPage"
                    ...
                    if (DBOX.getLastDialbox() == "cloud") {
                        ...
                        PDI.appendDialbox('normal', toIndex, {title: self.container.find('#webSiteTitle').val(), url: self.container.find('#webSiteUrl').val(), img: self.container.find('#webSiteLogo').val(), isApp: false, isNew: true})
                    } else {
                        PDI.insertDialbox('normal', {title: self.container.find('#webSiteTitle').val(), url: self.container.find('#webSiteUrl').val(), img: self.container.find('#webSiteLogo').val(), isApp: false, isNew: true})
                    }
                    if (self.container.find('#webSiteClassification').val() != cId) {
            ...
            logoImg.onload = function () {
                if ((150 / 90) < (logoImg.width / logoImg.height)) {
                    self.container.find('.logoBox .logo').css({'backgroundSize': '100% auto'})
                } else {
                    self.container.find('.logoBox .logo').css({'backgroundSize': 'auto 100%'})
                }
                self.container.find('.logoBox .logo').css({'backgroundImage': 'url(' + logoUrl + ')'})
            };
            logoImg.src = logoUrl
            ...
            if (!(_browser.msie && _browser.msie <= 9)) {
            ...
            for (var i = iStart + 1; i <= iStart + 30; i++) {
                self.container.find('.logoContainer').append($('<div class="logoLine"></div><div class="logoItem" style="background-image:url(' + urlImg + 'cloudapp/generalLogo/m/' + pad(i, 4) + '.png)"></div>').bind("click", function () {
                    ...
                    $(this).css({"backgroundImage": self.container.find('.logoBox .logo').css("backgroundImage"), "backgroundSize": self.container.find('.logoBox .logo').css("backgroundSize")});
                    self.container.find('.logoBox .logo').css({"backgroundImage": selectedLogo, "backgroundSize": selectedLogoSize});
                    self.container.find('.logoBox').removeClass("selected");
                    self.container.find('.logoContainer').hide()
                }))
            }
...
var createWebsite = $.createWebsite({"container": $('.createWebsite')});