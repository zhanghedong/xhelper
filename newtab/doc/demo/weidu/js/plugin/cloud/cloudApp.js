(function ($) {
    ...
    var cloudApp = (function (opt) {
        ...
        cloudApp.prototype = {installApps: [], container: '', init: function () {
            var self = this;
            self.installApps = [];
            ...
            if (ver.indexOf('.360') > -1) {
                ignoreAppDialboxs.push('oDownloads');
                ignoreAppDialboxs.push('oBookmarks');
            ...
            appItem.find('.itemWindow').unbind('click').bind('click', function () {
                if ($(this).siblings('.itemInstall').hasClass('selected')) {
                    ...
                    if (DBOX.getLastDialbox() == "cloud") {
                        ...
                        PDI.appendDialbox('normal', toIndex, {title: app.title, img: app.img, url: app.url, desc: app.desc, isApp: app.id, appType: app.type, isFixed: app.isFixed, isNew: true})
                    } else {
                        PDI.insertDialbox('normal', {title: app.title, img: app.img, url: app.url, desc: app.desc, isApp: app.id, appType: app.type, isFixed: app.isFixed, isNew: true})
                    }
...
var cloudApp = $.cloudApp({"container": $('.container.appList'), });