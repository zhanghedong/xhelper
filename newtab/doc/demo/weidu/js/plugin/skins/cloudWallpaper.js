(function ($) {
    ...
    var cloudWallpaper = (function (el, opt) {
        ...
        cloudWallpaper.prototype = {container: '', page: 1, num: 30, loading: false, category: 'Beauty', cacheData: {}, init: function () {
            var self = this;
            self.cacheData['All'] = [];
            ...
            $.getJSON(cwURL + 'index.json', function (data) {
                $.each(data, function (i, n) {
                    if (n != "") {
                        ...
                        self.container.parent().parent().find('.cloudWallpaperCategorys').append($('<div class="categoryItem" id="' + i + '">' + getI18nMsg('wallpaperCategory' + i) + '</div>').unbind('click').bind('click', function () {
                            self.container.parent().parent().find('.cloudWallpaperCategorys').find('.categoryItem').removeClass('selected');
                            $(this).addClass('selected');
                            self.initWallpaperContainer($(this).attr('id'), 1)
                        }))
                    }
                });
                self.container.parent().parent().find('.cloudWallpaperCategorys').append($('<div class="categoryItem" id="Used">' + getI18nMsg('wallpaperCategoryUsed') + '</div>').unbind('click').bind('click', function () {
                    self.container.parent().parent().find('.cloudWallpaperCategorys').find('.categoryItem').removeClass('selected');
                    $(this).addClass('selected');
                    self.initWallpaperContainer($(this).attr('id'), 1)
                }));
                shuffle(self.cacheData['All']);
                self.container.parent().parent().find('.cloudWallpaperCategorys').prepend($('<div class="categoryItem selected" id="All">' + getI18nMsg('wallpaperCategoryAll') + '</div>').unbind('click').bind('click', function () {
                    self.container.parent().parent().find('.cloudWallpaperCategorys').find('.categoryItem').removeClass('selected');
                    $(this).addClass('selected');
                    self.initWallpaperContainer($(this).attr('id'), 1)
                }));
            ...
            for (var i = start; i < end; i++) {
                if (typeof self.cacheData[self.category][i] != 'undefined') {
                    ...
                    var realsrc = cwURL + self.category + '/' + imgIndex + '.jpg';
                    ...
                    self.container.append($('<div class="cloudWallpaperItem' + (i % 5 == 0 ? " big" : "") + float + '" imgIndex="' + imgIndex + '"><img realsrc="' + realsrc + '" src="' + realsrc.replace(/\/(\d{4}\.)/, '/s_$1') + '" /><div class="cloudWallpaperItemDownload"></div>' + (self.category == "Used" ? '<div class="cloudWallpaperItemDelete" title="' + getI18nMsg("delete") + '"></div>' : '') + '<div class="cloudWallpaperItemBar"></div></div>').unbind('click').bind('click', function (e) {
                        ...
                        $(imgObj).bind('load', function () {
                            ...
                            var style = {"background": {"backgroundColor": tmp_color, "backgroundImage": "url(" + url + ")"}};
                            ...
                            if (self.cacheData['Used'].indexOf(thisSelf.attr('imgIndex')) == -1) {
                                ...
                                if (self.cacheData['Used'].length > 50) {
                                    self.cacheData['Used'].splice(self.cacheData['Used'].length - 1, self.cacheData['Used'].length - 50)
                                }
                            }
                            PDI.set('usedWallpaper', '', self.cacheData['Used']);
                            oauth.updateMsgId();
