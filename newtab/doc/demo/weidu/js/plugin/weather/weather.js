(function ($) {
    ...
    var weather = (function () {
        ...
        weather.prototype = {ip: '', city: '', cityID: '', defaultCityID: 2514815, isAuto: false, pre: false, weather: '', tempUnit: PDI.get("weather", "tempUnit"), calendar: '', dateline: 0, weatherCache: '', container: '', content: '', weekOptions: [getI18nMsg("calendarSun"), getI18nMsg("calendarMon"), getI18nMsg("calendarTue"), getI18nMsg("calendarWed"), getI18nMsg("calendarThu"), getI18nMsg("calendarFri"), getI18nMsg("calendarSat")], imageOptions: {"1:1": {0: [32, 33, 34, 36], 1: [27, 28], 2: [26, 29, 30, 44], 3: [9, 40], 4: [], 5: [11, 12, 37], 6: [3, 4, 38, 39, 45, 47], 7: [13, 14, 16, 40, 41, 42, 43, 46], 8: [17], 9: [25], 10: [0, 1, 2, 23, 24], 11: [19, 20, 21, 22], "5-7": [5, 6, 7, 18], "5-8": [8, 10, 35], "7-10": [15], "1-0": [31, 28, 30], }, "2:1": {}}, bgImageOptions: {0: [31, 32, 33, 34, 36], 1: [25, 26, 27, 28, 29, 30, 44], 3: [8, 9, 10, 11, 12, 17, 35, 37, 38, 39, 40, 45, 47], 7: [5, 6, 7, 13, 14, 15, 16, 18, 41, 42, 43, 46], 10: [0, 1, 2, 3, 4, 23, 24], 11: [19, 20, 21, 22, ]}, init: function (opt) {
            var self = this;
            self.weatherCache = "";
            ...
            if (self.cityID == '' || (self.isAuto && curTime >= self.dateline + 1 * 3600)) {
                ...
                $.ajax({url: urlImg + "myapp/weather/city/global/getAddress.php?t=" + curTime, success: function (result) {
                    ...
                    if (typeof ipInfo == 'object' && ipInfo.city) {
                        $.ajax({url: urlImg + "tianqi/city.php?city=" + encodeURIComponent(ipInfo.city) + "&type=yahoo&t=" + curTime, success: function (data) {
            ...
            $.ajax({url: urlImg + "myapp/weather/data/yahooData/index.php?cityID=" + self.cityID + "&u=" + self.tempUnit.toLowerCase() + "&t=" + new Date().getTime(), success: function (data) {
                if (data) {
            ...
            if (self.cityID != "" && curTime >= self.dateline + 1 * 3600) {
                self.getWeatherData(function (xmlDoc) {
                    if (xmlDoc.getElementsByTagName("title")[0].childNodes[0].nodeValue != 'Yahoo! Weather - Error') {
                        ...
                        self.weather = {text: self.weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("text").value, images: self.weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("code").value, temp: self.weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("high").value + getI18nMsg('weatherTempUnit' + self.tempUnit) + " ~ " + self.weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("low").value + getI18nMsg('weatherTempUnit' + self.tempUnit), wind: "Wind chill:" + self.weatherCache.getElementsByTagName(self.pre + "wind")[0].attributes.getNamedItem("chill").value + " Direction:" + self.weatherCache.getElementsByTagName(self.pre + "wind")[0].attributes.getNamedItem("direction").value + " Speed:" + self.weatherCache.getElementsByTagName(self.pre + "wind")[0].attributes.getNamedItem("speed").value};
                        ...
                        self.calendar = {year: weatherDateArray[2], month: weatherDateArray[1], date: weatherDateArray[0], day: '', weekDay: self.weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("day").value};
                        self.dateline = curTime;
                        PDI.set('weather', '', {city: self.city, cityID: self.cityID, isAuto: self.isAuto, weather: self.weather, tempUnit: self.tempUnit, calendar: self.calendar, dateline: self.dateline, ip: PDI.get("setup", "ip")});
                        fn();
                        self.initWeatherApp()
                    } else {
                        if (self.cityID != self.defaultCityID) {
            ...
            if (DBOX.width >= 180 && DBOX.height >= 90) {
                self.container.append($('<div class="weatherApp"><div class="preview"></div><div class="temp">' + self.weather.temp + '</div><div class="address">' + self.city + '</div><div class="lunarCalendar">' + self.calendar.date + " " + self.calendar.month + " [" + self.calendar.weekDay + ']</div><div class="weather">' + self.weather.text + '</div></div>').css({'marginTop': DBOX.titleShow == true ? "-61px" : "-45px"}))
            ...
            var weatherCacheData = {text: weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("text").value, images: weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("code").value, temp: weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("high").value + getI18nMsg('weatherTempUnit' + self.tempUnit) + " / " + weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("low").value + getI18nMsg('weatherTempUnit' + self.tempUnit), wind: "Direction: " + weatherCache.getElementsByTagName(self.pre + "wind")[0].attributes.getNamedItem("direction").value + " Speed: " + weatherCache.getElementsByTagName(self.pre + "wind")[0].attributes.getNamedItem("speed").value, date: weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("date").value, weekDay: weatherCache.getElementsByTagName(self.pre + "forecast")[0].attributes.getNamedItem("day").value};
            var weatherCacheDataTomorrow = {text: weatherCache.getElementsByTagName(self.pre + "forecast")[1].attributes.getNamedItem("text").value, images: weatherCache.getElementsByTagName(self.pre + "forecast")[1].attributes.getNamedItem("code").value, tempHigh: weatherCache.getElementsByTagName(self.pre + "forecast")[1].attributes.getNamedItem("high").value + getI18nMsg('weatherTempUnit' + self.tempUnit), tempLow: weatherCache.getElementsByTagName(self.pre + "forecast")[1].attributes.getNamedItem("low").value + getI18nMsg('weatherTempUnit' + self.tempUnit), date: weatherCache.getElementsByTagName(self.pre + "forecast")[1].attributes.getNamedItem("date").value, weekDay: weatherCache.getElementsByTagName(self.pre + "forecast")[1].attributes.getNamedItem("day").value};
            var weatherForecast = '';
            ...
            if (cityID != "") {
                ...
                $.ajax({url: urlImg + "myapp/weather/city/global/getAddress.php?t=" + t, success: function (result) {
                    var ipInfo = JSON.parse(result);
                    if (typeof ipInfo == 'object' && ipInfo.city) {
                        $.ajax({url: urlImg + "tianqi/city.php?city=" + encodeURIComponent(ipInfo.city) + "&type=yahoo&t=" + t, success: function (data) {
                            self.cityID = self.defaultCityID;
                            data = JSON.parse(data);
                            if (typeof data == 'object') {
                                var cityList = [];
