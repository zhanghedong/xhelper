(function ($) {
    ...
    var setup = (function () {
        var _dialBoxSpacingFun = '', _dialBoxWidthFun = '', _dialBoxHeightFun = '', _dialBoxRadiusFun = '', _dialBoxOpacityFun = '', _dialBoxQBCWidthFun = '';
        var setup = function () {
        };
        setup.prototype = {content: '', init: function (force) {
            var self = this;
            if (force == true) {
                self.content = ""
            }
            ...
            self.content.find(".dialBoxSettingsContainer .more").bind("click", function () {
                self.content.find(".settingBox").css({"left": "-760px", "opacity": 0});
                setTimeout(function () {
                    self.content.find(".dialBoxSettingsBox").css({"left": "0px", "opacity": 1})
                }, 0)
            });
            self.content.find(".back").bind("click", function () {
                self.content.find(".dialBoxSettingsBox").css({"left": "760px", "opacity": 0});
                setTimeout(function () {
                    self.content.find(".settingBox").css({"left": "0px", "opacity": 1})
                }, 0)
            });
            ...
            self.content.find(".dataAction").bind("click", function () {
                if ($(this).hasClass('import')) {
                    ...
                    if (oauth.backup()) {
                        ...
                        $.post(urlImg + "weidu/wc.json.php", {a: "export", data: JSON.stringify(oauthData[oauth.oauthId])}, function (result) {
                            if (!result || typeof(result) != 'string' || result.substr(0, 5) == 'ERROR') {
                                showNotice(getI18nMsg('exportError'));
                                return
                            }
                            var hiddenIframe = self.content.find(".loginContainer #hiddenIframe");
                            if (hiddenIframe.length == 0) {
                                hiddenIframe = $('<iframe src="about:blank" id="hiddenIframe" style="display:none" height="0" width="0"></iframe>');
                                self.content.find(".loginContainer").append(hiddenIframe)
                            }
                            hiddenIframe.attr('src', urlImg + "downloadData.php?url=weidu/" + result)
                        })
                    }
                }
            });
            self.content.find("#importData").bind('change', function () {
                ...
                reader.onload = function (e) {
                    $.post(urlImg + "weidu/wc.json.php", {a: "import", lang: ui_locale, data: this.result}, function (result) {
            ...
            self.content.find("input[type='checkbox']").bind('click', function () {
                ...
                if ($(this).attr('id') == 'contextMenusSwitch') {
                    if ($(this).attr('checked') == null) {
                        chrome.contextMenus.create({id: "addToDialbox", title: getI18nMsg("contextMenusAddToDialbox")}, function () {
                        })
                    } else {
            ...
            var skipNum = {5: true, 7: true, 10: true, 11: true, 13: true, 14: true, 17: true, 19: true, 22: true, 23: true, 26: true, 29: true, 31: true};
            ...
            var dialBoxStyles = {'default': [((screenWidth / screenHeight >= 1.25) ? 9 : 8), 12, ((screenWidth / screenHeight >= 1.25) ? 16 : 15), 20], 'square': [8, 12, 15, 21], 'seamless': [9, 12, 16, 20]}, dialBoxStyleContents = {'default': '', 'square': '', 'seamless': ''};
            ...
            if (oauth.oauthId) {
                loginButtonHtml += '<div class="loginButton"><img src="' + urlImg + 'oauth/img/login_' + oauth.oauthSource + '.png" width="87" height="32" /></div><div class="userContent"><div class="userContentArrowBorder"></div><div class="userContentArrow"></div><span class="oauthCode">' + oauth.oauthCode + '</span><span class="oauthSyn">[ ' + getI18nMsg('synchronize') + ' ]</span><span class="oauthLogout">[ ' + getI18nMsg('logout') + ' ]</span></div>'
            } else {
                $.each(_config["oauthType"], function (i, n) {
                    loginButtonHtml += '<div class="loginButton" style="display:' + (i < 3 ? "block" : "none") + '"><a href="' + urlImg + 'oauth/login.php?sign=' + n + '&oauthKey=' + PDI.get("setup", "oauthKey") + '&referer=' + window.location.protocol + '//' + window.location.hostname + window.location.pathname + '&hash=synchronize&t=' + new Date().getTime() + '" target="_self"><img src="' + urlImg + 'oauth/img/login_' + n + '.png" width="87" height="32" title="' + getI18nMsg(n + 'LoginTypeTitle') + '"/></a></div>'
                });
                if (_config["oauthType"].length > 3) {
